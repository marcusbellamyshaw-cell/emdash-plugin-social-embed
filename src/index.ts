		...
		routes: {
			...
			oembed: {
				...
				handler: async (routeCtx: unknown, ctx: PluginContext) => {
					...
					const metaAppSecret = (await ctx.kv.get<string>("settings:metaAppSecret")) ?? undefined;
					...
					if (!embed) {
						return {
							...
							platform,
							// Tell the component why so it can show a helpful message
							needsCredentials:
			...
			admin: {
				handler: async (routeCtx: unknown, ctx: PluginContext) => {
								...
								},
								{
									...
									fields: [
										...
										},
										{
											...
											placeholder: currentAppId ? "Leave blank to keep existing secret" : "Enter your app secret",
										},
									],
								},
								{
					...
					if (
						...
						// Only update the secret if a new value was entered
