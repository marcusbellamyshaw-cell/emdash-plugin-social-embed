...
function repeater(actionId, label, fields, opts) {
	return {
		...
		label,
		fields,
		...opts?.itemLabel !== void 0 && { item_label: opts.itemLabel },
		...opts?.minItems !== void 0 && { min_items: opts.minItems },
		...opts?.maxItems !== void 0 && { max_items: opts.maxItems },
		...opts?.initialValue !== void 0 && { initial_value: opts.initialValue }
	};
}
function mediaPicker(actionId, label, opts) {
	return {
		...
		action_id: actionId,
		label,
		...opts?.mimeTypeFilter !== void 0 && { mime_type_filter: opts.mimeTypeFilter },
		...opts?.initialValue !== void 0 && { initial_value: opts.initialValue },
		...opts?.placeholder !== void 0 && { placeholder: opts.placeholder }
	};
}
...
function tabBlock(panels, opts) {
	return {
		type: "tab",
		panels,
		...opts?.defaultTab !== void 0 && { default_tab: opts.defaultTab },
		...opts?.blockId !== void 0 && { block_id: opts.blockId }
	};
}
function empty(opts) {
	return {
		...
		title: opts.title,
		...opts.description !== void 0 && { description: opts.description },
		...opts.commandLine !== void 0 && { command_line: opts.commandLine },
		...opts.size !== void 0 && { size: opts.size },
		...opts.actions !== void 0 && { actions: opts.actions },
		...opts.blockId !== void 0 && { block_id: opts.blockId }
	};
}
function accordion(opts) {
	return {
		...
		label: opts.label,
		blocks: opts.blocks,
		...opts.defaultOpen !== void 0 && { default_open: opts.defaultOpen },
		...opts.blockId !== void 0 && { block_id: opts.blockId }
	};
}
const blocks = {
...
function validateElement(value, path, errors) {
	...
	switch (type) {
		...
		case "repeater": {
			if (!Array.isArray(value.fields)) errors.push({
			...
			else if (value.fields.length === 0) errors.push({
				path: `${path}.fields`,
				...
				if (isRecord(sub) && typeof sub.type === "string" && !REPEATER_SUB_FIELD_TYPES.has(sub.type)) {
					errors.push({
						path: `${subPath}.type`,
			...
			if (value.item_label !== void 0 && typeof value.item_label !== "string") errors.push({
				path: `${path}.item_label`,
				message: "Field 'item_label' must be a string"
			});
			const minOk = value.min_items === void 0 || typeof value.min_items === "number" && Number.isInteger(value.min_items) && value.min_items >= 0;
			if (!minOk) errors.push({
				path: `${path}.min_items`,
				message: "Field 'min_items' must be a non-negative integer"
			});
			const maxOk = value.max_items === void 0 || typeof value.max_items === "number" && Number.isInteger(value.max_items) && value.max_items >= 0;
			if (!maxOk) errors.push({
				path: `${path}.max_items`,
				message: "Field 'max_items' must be a non-negative integer"
			});
			if (minOk && maxOk && value.min_items !== void 0 && value.max_items !== void 0 && typeof value.min_items === "number" && typeof value.max_items === "number" && value.min_items > value.max_items) errors.push({
				path: `${path}.min_items`,
				message: "Field 'min_items' must be less than or equal to 'max_items'"
			});
			if (value.initial_value !== void 0) {
				if (!Array.isArray(value.initial_value)) errors.push({
					path: `${path}.initial_value`,
					message: "Field 'initial_value' must be an array"
				});
				else for (let i = 0; i < value.initial_value.length; i++) if (!isRecord(value.initial_value[i])) errors.push({
					path: `${path}.initial_value[${i}]`,
		...
		case "media_picker":
			if (value.mime_type_filter !== void 0) {
				if (typeof value.mime_type_filter !== "string") errors.push({
					path: `${path}.mime_type_filter`,
					message: "Field 'mime_type_filter' must be a string"
				});
				else if (!MEDIA_PICKER_MIME_FILTER_RE.test(value.mime_type_filter)) errors.push({
					path: `${path}.mime_type_filter`,
					message: "Field 'mime_type_filter' must be an image MIME type or prefix, e.g. 'image/' or 'image/png' (wildcards and non-image types are not supported)"
				});
			}
			if (value.initial_value !== void 0 && typeof value.initial_value !== "string") errors.push({
				path: `${path}.initial_value`,
...
function validateBlock(value, path, errors) {
	...
	switch (type) {
		...
		case "empty":
			...
			if (value.command_line !== void 0 && typeof value.command_line !== "string") errors.push({
				path: `${path}.command_line`,
				message: "Field 'command_line' must be a string if provided"
			});
			if (value.size !== void 0 && (typeof value.size !== "string" || !EMPTY_SIZES.has(value.size))) errors.push({
				path: `${path}.size`,
			...
			else for (let i = 0; i < value.actions.length; i++) validateElement(value.actions[i], `${path}.actions[${i}]`, errors);
			break;
		case "accordion":
			if (typeof value.label !== "string") errors.push({
				path: `${path}.label`,
			...
			if (!Array.isArray(value.blocks)) errors.push({
				path: `${path}.blocks`,
				message: "Required field 'blocks' must be an array"
			});
			else for (let i = 0; i < value.blocks.length; i++) validateBlock(value.blocks[i], `${path}.blocks[${i}]`, errors);
			if (value.default_open !== void 0 && typeof value.default_open !== "boolean") errors.push({
...
//# sourceMappingURL=validation-Dq-a7CXm.js.map